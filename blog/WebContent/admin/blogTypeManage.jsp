...
// 	var url;
    ...
    $(function () {
        ...
        $("#dg").datagrid({
            //请求数据的url
            url: 'http://localhost:8080/blog/admin/blogType/list',
            //载入提示信息
            loadMsg: 'loading...',
            ...
            toolbar:[{
                ...
                handler: function () {  //回调函数
                	...
                	//将URL设置为添加
                	url="http://localhost:8080/bloge/admin/blogType/save";
                }
            },'-',{
                iconCls: 'icon-edit',
                text: '修改',
                handler: function () {
                      ...
                      alert("selectedRows.lenght:"+selectedRows.lenght);
                      //确保被选中行只能为一行
                      if(selectedRows.lenght != 1){
                      ...
                      url = "http://localhost:8080/blog/admin/blogType/save?blog_id=" + row.blog_id;
                    }
            },'-',{
                iconCls: 'icon-edit',
                text: '删除',
...
//                 	debugger;
                	//获取选中要删除的行
                	var selectedRows=$("#dg").datagrid("getSelections");
                	var idsStr=JSON.stringify(selectedRows);
                	/* $.each(JSON.parse(idsStr),function(i,n)
                		{
                			alert('索引:'+ i +'对应值为：'+n.blog_id);
                		}); */

                	if(selectedRows.lenght==0){
                		alert("请选择要删除的数据:"+selectedRows.lenght);
                	...
                	$.messager.confirm("系统提示", "<font color=red>您确定要删除选中的"+selectedRows.length+"条数据么？</font>", function(r) {
                        if(r) {
                            $.post("http://localhost:8080/blog/admin/blogType/delete",
            ...
            frozenColumns:[[
                ...
                {field:'blog_id',title:'编号',width:200}    //id字段
            ]],
            columns:[[
	...
	function saveBlogType(){
		$("#fm").form("submit",{
			url:"http://localhost:8080/blog/admin/blogType/save",
			onSubmit:function(){
				return $(this).form("validate");
			},//进行验证，通过才让提交
			success:function(result){
				var res=eval("("+result+")");//将json格式的result转换成js对象
// 				alert(res.success);
				if(res.success){
